...
module.exports = ({ host, hexData, log, name, debug }) => {
    ...
    if (!device) {
        ...
        return log.debug(`\x1b[31m[ERROR] \x1b[0m${name} sendData (no device found at ${host})`);
    }

    if (!device.sendData) return log.warn(`\x1b[31m[ERROR] \x1b[0mThe device at ${device.host.address} (${device.host.macAddress}) doesn't support the sending of IR or RF codes.`);
    if (hexData.includes('5aa5aa555')) return log.warn(`\x1b[31m[ERROR] \x1b[0mThis type of hex code (5aa5aa555...) is no longer valid. Use the included "Learn Code" accessory to find new (decrypted) codes.`);

    const hexDataBuffer = new Buffer(hexData, 'hex');
    device.sendData(hexDataBuffer, debug, hexData);

    log.debug(`${name} sendHex (${device.host.address}; ${device.host.macAddress}) ${hexData}`);
}
