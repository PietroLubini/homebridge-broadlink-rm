...
class AirConAccessory extends BroadlinkRMAccessory {
    ...
    getTargetModeName() {
        return this.HeatingCoolingConfigKeys[this.state.targetHeatingCoolingState];
    }
    ...
    updateAccessories(accessories) {
        ...
        log.debug(`${name} Linking autoSwitch "${autoSwitchName}"`)

        const autoSwitchAccessories = accessories.filter(accessory => accessory.name === autoSwitchName);

        if (autoSwitchAccessories.length === 0) return log.warn(`${name} No accessory could be found with the name "${autoSwitchName}". Please update the "autoSwitchName" value or add a matching switch accessory.`);
    ...
    setTargetTemperature(hexData, previousValue) {
        ...
        if (state.targetTemperature < minTemperature) return log.warn(`The target temperature (${this.targetTemperature}) must be more than the minTemperature (${minTemperature})`);
        if (state.targetTemperature > maxTemperature) return log.warn(`The target temperature (${this.targetTemperature}) must be less than the maxTemperature (${maxTemperature})`);
    ...
    async setTargetHeatingCoolingState(hexData, previousValue) {
        const { HeatingCoolingConfigKeys, HeatingCoolingStates, config, data, serviceManager, state } = this;
        const { preventResendHex, defaultCoolTemperature, defaultHeatTemperature } = config;
    ...
    async sendTemperature(temperature, previousTemperature) {
        const { HeatingCoolingStates, config, log, name, state } = this;
        const { preventResendHex, ignoreTemperatureWhenOff, sendTemperatureOnlyWhenOff } = config;

        log.debug(`${name} Potential sendTemperature (${temperature})`);
        ...
        const { hexData, finalTemperature, targetModeName } = this.getTemperatureHexData(temperature);
        ...
        if (!state.currentHeatingCoolingState && !state.targetHeatingCoolingState && ignoreTemperatureWhenOff) {
            log.debug(`${name} Ignoring sendTemperature due to "ignoreTemperatureWhenOff": true`);
        ...
        this.log.debug(`${name} sendTemperature (${state.targetTemperature})`);
        ...
        this.log.debug(`${name} sendTemperature (set mode to ${mode})`);

        state.targetHeatingCoolingState = HeatingCoolingStates[targetModeName];
        this.updateServiceCurrentHeatingCoolingState(HeatingCoolingStates[targetModeName]);
    ...
    getTemperatureHexData(temperature) {
        const { config, data, name, log } = this;
        const { defaultHeatTemperature, defaultCoolTemperature, heatTemperature } = config;
        let targetModeName = this.getTargetModeName();

        let finalTemperature = temperature;
        let hexData = data[`${targetModeName}${temperature}`];
        ...
        if (!hexData) {
            ...
            hexData = data[`${targetModeName}${defaultTemperature}`];

            assert(hexData, `\x1b[31m[CONFIG ERROR] \x1b[0m You need to provide a hex code for the following temperature:
        \x1b[33m{ "${targetModeName}${temperature}": { "data": "HEXCODE", "pseudo-mode" : "heat/cool" } }\x1b[0m 
        or provide the default temperature:
        \x1b[33m { "${targetModeName}${defaultTemperature}": { "data": "HEXCODE", "pseudo-mode" : "heat/cool" } }\x1b[0m`);


            this.log.debug(`${name} Update to default temperature (${defaultTemperature})`);
            finalTemperature = defaultTemperature;
        }

        return { finalTemperature, hexData, targetModeName };
    }
    ...
    async checkTurnOnWhenOff() {
        ...
        if (state.currentHeatingCoolingState === Characteristic.TargetHeatingCoolingState.OFF && config.turnOnWhenOff) {
            log.debug(`${name} sendTemperature (sending "on" hex before sending temperature)`);
    ...
    async monitorTemperature() {
        ...
        log.debug(`${name} monitorTemperature`);
    ...
    onTemperature(temperature) {
        ...
        log.debug(`${name} onTemperature (${temperature})`);

        if (temperature > config.maxTemperature) {
            log.warn(`\x1b[35m[INFO]\x1b[0m Reported temperature (${temperature}) is too high, setting to \x1b[33mmaxTemperature\x1b[0m (${maxTemperature}).`)
            temperature = config.maxTemperature
        }

        if (temperature < config.minTemperature) {

            log.warn(`\x1b[35m[INFO]\x1b[0m Reported temperature (${temperature}) is too low, setting to \x1b[33mminTemperature\x1b[0m (${minTemperature}).`)
    ...
    addTemperatureCallbackToQueue(callback) {
        ...
        if (Object.keys(this.temperatureCallbackQueue).length > 1) {
            if (state.currentTemperature) {
                log.debug(`${name} addTemperatureCallbackToQueue (clearing previous callback, using existing temperature)`);
        ...
        if (!device || device.state === 'inactive') {
            if (device && device.state === 'inactive') {
                log.warn(`${name} addTemperatureCallbackToQueue (device no longer active, using existing temperature)`);
            }
        ...
        device.checkTemperature();
        log.debug(`${name} addTemperatureCallbackToQueue (requested temperature from device, waiting)`);
    }
    ...
    updateTemperatureFromFile() {
        ...
        log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile reading file: ${temperatureFilePath}`);

        fs.readFile(temperatureFilePath, 'utf8', (err, temperature) => {

            if (err) {
                log.debug(`\x1b[31m[ERROR] \x1b[0m${name} updateTemperatureFromFile\n\n${err.message}`);

                return;
            }

            if (temperature === undefined || temperature.trim().length === 0) {
                log.debug(`\x1b[31m[ERROR] \x1b[0m${name} updateTemperatureFromFile (no temperature found)`);

                temperature = (state.currentTemperature || 0);
            }

            log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile (file content: ${temperature.trim()})`);

            temperature = parseFloat(temperature);

            log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile (parsed temperature: ${temperature})`);
    ...
    getCurrentTemperature(callback) {
        ...
        if (pseudoDeviceTemperature !== undefined) {
            log.debug(`${name} getCurrentTemperature (using pseudoDeviceTemperature ${pseudoDeviceTemperature} from config)`);
    ...
    async checkTemperatureForAutoOnOff(temperature) {
        ...
        if (this.shouldIgnoreAutoOnOff) {
            log.debug(`${name} checkTemperatureForAutoOn (ignore within ${minimumAutoOnOffDuration}s of previous auto-on/off due to "minimumAutoOnOffDuration")`);
        ...
        if (!this.isAutoSwitchOn()) {
            log.debug(`${name} checkTemperatureForAutoOnOff (autoSwitch is off)`);

            return;
        }

        log.debug(`${name} checkTemperatureForAutoOnOff`);

        if (autoHeatTemperature && temperature < autoHeatTemperature) {
            this.state.isRunningAutomatically = true;

            log.debug(`${name} checkTemperatureForAutoOnOff (${temperature} < ${autoHeatTemperature}: auto heat)`);
            ...
            log.debug(`${name} checkTemperatureForAutoOnOff (${temperature} > ${autoCoolTemperature}: auto cool)`);
            serviceManager.setCharacteristic(Characteristic.TargetHeatingCoolingState, Characteristic.TargetHeatingCoolingState.COOL);
        } else {
            log.debug(`${name} checkTemperatureForAutoOnOff (temperature is ok)`);

            if (this.state.isRunningAutomatically) {
                this.isAutomatedOff = true;

                log.debug(`${name} checkTemperatureForAutoOnOff (auto off)`);
    ...
    onMQTTMessage(identifier, message) {
        ...
        if (identifier !== 'unknown' && identifier !== 'temperature') {
            log.debug(`\x1b[31m[ERROR] \x1b[0m${name} onMQTTMessage (mqtt message received with unexpected identifier: ${identifier}, ${message.toString()})`);
        ...
        log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} onMQTTMessage (raw value: ${temperature})`);
        ...
        if (temperature === undefined || (typeof temperature === 'string' && temperature.trim().length === 0)) {
            log.debug(`\x1b[31m[ERROR] \x1b[0m${name} onMQTTMessage (mqtt temperature temperature not found)`);

            return;
        }

        log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} onMQTTMessage (raw value 2: ${temperature.trim()})`);

        temperature = parseFloat(temperature);

        log.debug(`\x1b[33m[DEBUG]\x1b[0m ${name} onMQTTMessage (parsed temperature: ${temperature})`);
